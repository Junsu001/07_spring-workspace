...
<body>
 		...
 		$("input[type=file]").on('change', function(evt){
			...
			let totalSize = 0;
 			...
 			for(let i=0; i<files.length; i++){
 				if(files[i].size > 10 * 1024 * 1024){ // 첨부파일 한개의 크기가 10MB 초과할경우
 				...
 				if(totalSize > 100 * 1024 * 1024){ // 전체 첨부파일 사이즈가 100MB 초과할경우
 	...
 	<h2>1. 한 개의 첨부파일 업로드 테스트</h2><!-- 파일 자체를 넘기려면 enctype구문을 써줘야함 -->
 	<form action="${contextPath}/board/insert1.do" method="post" enctype="multipart/form-data"> 
 		게시글 제목: <input name="boardTitle" type="text"><br>
	...
	<form action="${contextPath}/board/insert2.do" method="post" enctype="multipart/form-data"> 
	 		...
	 		<!-- CommonsFileupload의 경우 multiple을 사용하면 다중파일선택 가능 -->
	...
	<div id="async_test"> 
	 		...
	 		첨부파일 : <input type="file" id="file"> <br> 
	 		<!-- CommonsFileupload의 경우 multiple을 사용하면 다중파일선택 가능 -->
	 		<label>각 첨부파일 사이즈는 10MB 이하, 총 100MB 이하여야 됩니다.</label> <br><br>
      ...
      $(function(){
         $("#submit").on("click", function(){
            ...
            $.ajax({
               ...
               type: 'post',
               data: formData,
	     		  // 첨부파일 보낼때 아래 두 구문이 필수 (text값 넘길때는 필요없음)
   ...
   <p>
		...
		> 에디터 api : summernote, ckeditor, 등등 summernote가 비교적 간단  	 
   </p>
		...
		$(document).ready(function() {
			  $('#summernote').summernote({
				  ...
				  callbacks: {
					  ...
					  // 이미지 업로드시 간소화된 태그로 보여주게 함. 하면 좋음
	...
	<script>
		function fn_selectBoard(){
			$.ajax({
				...
				type: 'get',
				data: $('#editor_search').serialize(), // serialize()하면 "boardNo=xx" 문자열로 바뀜
