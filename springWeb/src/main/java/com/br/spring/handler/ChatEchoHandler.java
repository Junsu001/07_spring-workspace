...
public class ChatEchoHandler extends TextWebSocketHandler{
	...
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		...
		*/
		...
		for(WebSocketSession sss : sessionList) {
			String msg = "entry|" + ((MemberDto)session.getAttributes().get("loginUser")).getUserName() + "님이 입장하였습니다."; 
	...
	public void handleMessage(WebSocketSession session, WebSocketMessage<?> message) throws Exception {
		...
		*/
		...
		for(WebSocketSession sss : sessionList) {
			// 메세지유형(chat/entry/exit)|채팅방에 띄워주고자하는 메세지내용|발신자아이디|...
			String msg = "chat|" + message.getPayload() + "|" + ((MemberDto)session.getAttributes().get("loginUser")).getUserName();
	...
	public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
		 ...
		 * log.debug("WebSocketSession 객체 : {}", session); log.debug("session id: {}",
		 * session.getId()); log.debug("현재 채팅방에서 나간 회원: {}",
		 * session.getAttributes().get("loginUser"));
		 */
		
		sessionList.remove(session);
		
		for(WebSocketSession sss : sessionList) {
			String msg = "exit|" + ((MemberDto)session.getAttributes().get("loginUser")).getUserName() + "님이 퇴장하였습니다."; 
			sss.sendMessage(new TextMessage(msg));
		}
