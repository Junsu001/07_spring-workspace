...
public class BoardScheduler {
	...
	/*
	 * * Scheduler
	 *   특정 날짜 | 특정 시간 | 일정주기마다 뭔가의 작업을 묵시적으로 실행시켜주는 주체
	 *   즉, 별도의 액션이 없어도 알아서 특정시간이 되면 자동 실행됨
	 *   * Spring에서 Scheduler
	 *   1) 매번 묵시적으로 실행시키고자 하는 작업들을 정의할 클래스 만들기 (빈 등록되야됨)
	 *   2) 해당 클래스 내에 각 작업별 메소드 작성
	 *      ㄴ 반환형은 void
	 *      ㄴ 매개변수 없이
	 *      ㄴ @Scheduled 부여하기
	 *   3) servlet-context.xml에서 스케줄링 사용을 위해 task 추가하기
	 *     
	 *   * Crom 표현식
	 ...
	 *      *		: 모든조건 (초|분|시|일|월)
	 *      L 		: 마지막 (일|요일)
	 *      W		: 가장가까운 평일 (일) ex) 10W ==> 10일이 평일일경우(10일에 실행) / 10일이 토요일인 경우 (9일에 실행) / 10일이 일요일인 경우 (11일에 실행)
	 *      #		: 몇주째인지 (요일)    ex) 3#2 ==> 수요일 2째주 
	 *      /		: 주기 (시작시간/단위) ex) 분 자리에 0/10 ==> 0분부터 10분 간격으로
	 *      -		: 범위 				   ex) 시 자리에 1-3 ==> 1시 , 2시 , 3시
	 *      ,		: 여러개 지정
	 *	
	 *   4) 예시
	 *   	0 0 * * * * 		: 매일 모든 시간마다 ( 1시 , 2시, 3시, 4시 ... )
	 *      0 0 5 * * *			: 매일 5시 마다       
