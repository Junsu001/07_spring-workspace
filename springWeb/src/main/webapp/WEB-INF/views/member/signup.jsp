 ...
 <div class="container p-3">
        ...
        <section class="row m-3" style="min-height: 500px">
          ...
          <div class="container border p-5 m-4 rounded">
            ...
            /* 각 항목 값들이 유효한 값으로 입력 되면 true로 변경 */
            let idResult = false;
            let pwdResult = false;
            let pwdEqualResult = false;
            let nameResult = false;
            
            //input요소들 유효성 검사 때 마다 회원가입 버튼 활성화 or 비활성화 처리 해주는 함수
            function validate(){
            	   console.log(idResult, pwdResult, pwdEqualResult, nameResult);
               if(idResult && pwdResult && pwdEqualResult && nameResult){
                  //모든 값들이 다 유효 값으로 입력 됐을 경우
                  //id가 signup_form인 폼에서 타입이 submit인 요소
                  $('#signup_form :submit').removeAttr('disabled');//버튼 활성화
               }else {
          		...
          		function noValueCheck($input, $result){ // 비교할 input요소객체, 메세지출력되는 div 요소객체
          			if($input.val().trim().length == 0){ // 값이 비어 있을 경우
                    ...
                    $("#signup_form #userId").on("keyup",function(){
                       ...
                       idResult = noValueCheck($(this),$("#idCheck_result"))
                                                        && regExpCheck($(this),$("#idCheck_result")
                                                                                         , regExp,"사용가능한 아이디입니다."
                                                                                         ,"영문, 숫자 포함 5~12글자로 작성해주세요.");
                       
                       if(idResult){
                          $.ajax({
                             url: '${contextPath}/member/idcheck.do',
                             async: false,//비동기처리(요청보내놓고 값 받기도 전에 유효성 true처리 되지 않게)
                             data: "checkId="+$(this).val(),
                             success: function(resData){
                                if(resData == "NNNNN"){
                                   $("#idCheck_result").removeClass("uncheck usable")
                                                                 .addClass("unusable")
                                                                 .text("중복된 아이디가 존재 합니다. 다시 입력 해 주세요.");
                                   idResult= false;
                                }//if
                             }
                          })
                          ...
                          validate();
								...
								pwdResult =	noValueCheck( $(this), $('#pwdCheck_result') )
															&& regExpCheck( $(this), $("#pwdCheck_result"), regExp, '사용가능한 비밀번호입니다.'
