			...
			<div class="container border p-5 m-4 rounded">
				...
				<c:if test="${ not empty loginUser }">
					...
					<a class="btn btn-secondary" style="float: right"
						href="${ contextPath }/board/regist.do">글쓰기</a>
					<br>
				</c:if>
				<br>
						...
						<c:choose>
							...
							<c:otherwise>
								<c:forEach var="b" items="${ list }">
									<tr
										onclick='location.href="${contextPath}/board/${ loginUser.userId eq b.boardWriter ? "detail.do" : "increase.do" }?no=${ b.boardNo }";'>
										<td>${ b.boardNo }</td>
										<td>${ b.boardTitle }</td>
										<td>${ b.boardWriter }</td>
										<td>${ b.count }</td>
				...
				<ul id="paging_area"
					...
					<li class="page-item ${ pi.currentPage == 1 ? 'disabled' : '' }">
						<a class="page-link"
						href="${ contextPath }/board/list.do?page=${pi.currentPage-1}">Previous</a>
					</li>
					...
					<c:forEach var="p" begin="${ pi.startPage }" end="${ pi.endPage }">
						...
						<li class="page-item ${ pi.currentPage == p ? 'active' : '' }">
							<a class="page-link"
							href="${ contextPath }/board/list.do?page=${p}">${ p }</a>
						</li>
					</c:forEach>
					...
					<li
						...
						<!--  다음버튼 --> <a class="page-link"
						href="${ contextPath }/board/list.do?page=${pi.currentPage+1}">Next</a>
					</li>
				</ul>
				...
				<c:if test="${ not empty search }">
					<script>
	                  ...
	                  // 검색후의 페이징바 클릭시 검색 form 을 강제로 submit (단, 페이지번호는 현재 클릭한 페이지번호로 바꿔서)
