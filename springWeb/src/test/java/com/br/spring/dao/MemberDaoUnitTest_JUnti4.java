 ...
 * - assert(단정메소드)로 테스트 케이스의 수행결과를 판별한 수 있음	
...
public class MemberDaoUnitTest_JUnti4 {
	 ...
	 * 	테스트 케이스 작성 패턴
	 * 
	 * given (준비) : 어떤 데이터를 가지고 테스트할건지
 	 * when (실행) : 어떤 메소드를 실행시킬건지 
	 * then (검증) : 어떤 데이터를 가지고 어떤 메소드 실행시 어떤 결과가 나와야되는지
	 * 			ㄴ assertEquals ( 예상값, 실행값 ) : 즉, 실행값이 예상값과 일치하는지 확인해주는 메소드
	...
	public void test01_로그인테스트() {
		...
		// given 필요한 데이터만 builder객체에 담는다
		MemberDto m = MemberDto.builder().userId("admin01")
										 .userPwd("1234")
										 .build();
