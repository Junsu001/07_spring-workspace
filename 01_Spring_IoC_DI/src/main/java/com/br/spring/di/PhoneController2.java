...
public class PhoneController2 {
	 ...
	 *		//= new PhoneServiceMoblicImpl(); -- 모바일 개발 당시
	 *
	 ...
	 *		 }
	 *
	 *		 A 클래스에서 B 클래스를 필드로 사용할 경우 연관관계 형성
	 *  	 만일 B 클래스가 변경(클래스명, 생성자 또는 메소드 수정) 될 경우
	 *  	 A 클래스도 일부 수정해야되는 상황 발생
	 *  	 => "두 객체간에 결합도가 높다" 라고 표현
	 *  	 
	 *  * 스프링의 IoC + DI를 활용해서 해결 가능
	 *    1) 개발에 사용할 서비스 클래스를 bean으로 등록 (xml | java | MVC용 어노테이션)
	 *    2) DI를 통해 객체를 주입받아 사용 (@Autowired)
	 *    
	 *    => 사용할 객체의 클래스가 변경돼도 해당 객체를 참조하는 클래스에서
	 *       수정할 필요가 없음 => 결합도가 낮아짐
	 *     
	 *    
	 */
	...
	/*
	private PhoneService pService 
		//= new PhoneServiceWebImpl();
		 = new PhoneServiceMobileImpl();
	 */
